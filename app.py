"""
수능 한파는 정말 있을까? - 데이터로 검증하는 도시 전설
================================================
서울(관측지점 108)의 1907~2026년 기온 데이터로
"수능날은 유독 춥다"는 통념을 통계적으로 검증합니다.

교육철학: "AI 제안, 인간 결정"
   AI/코드는 분석을 도와주지만, '한파가 있다/없다'의 해석은
   학생이 데이터를 직접 보고 내려야 합니다.
"""
...
with tab5:
    ...
    **Q5.** 만약 "수능 한파는 미신이다"라는 결론을 SNS에 올린다면, 어떤
    ...
    > 🎓 **수업에서의 의미**
    >
    > "AI 제안, 인간 결정"의 핵심이 여기에 있습니다. AI와 코드는 분석을
    > 빠르게 해주지만, "이 결과가 무엇을 뜻하는가, 어떻게 행동할 것인가"는
    > 학생이 데이터를 직접 보고 내려야 할 판단입니다. **데이터를 의심하는
    > 힘**, **'그럴듯한 이야기'에 속지 않는 힘**이야말로 이 수업의 진짜
    > 목표입니다.
    """)
